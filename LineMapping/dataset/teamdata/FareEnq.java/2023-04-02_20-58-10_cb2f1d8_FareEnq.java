...
//			String pWord = ck[1].getValue();
			if (!uName.equals("") || uName != null) {
				try {
					...
					PreparedStatement ps = con.prepareStatement("Select * from train where from_stn LIKE ? and to_stn LIKE ?");
					ps.setString(1, "%" + req.getParameter("fromstation").toUpperCase() + "%");
					ps.setString(2, "%" + req.getParameter("tostation").toUpperCase() + "%");
					...
					if (rs.next()) {
						...
						pw.println("<div class='main'><p1 class='menu'>Fare for Trains BetWeen Station "
								+ req.getParameter("fromstation") + " and " + req.getParameter("tostation")
								+ " is <p2 class=\"red\">RS " + rs.getLong("fare") + "</p2></p1></div>");
