			...
			if (!uName.equals("") || uName != null) {
				try {
					...
					PreparedStatement ps = con
							.prepareStatement("Select * from train where tr_no=? and from_stn=? and to_stn=?");
					ps.setLong(1, Long.parseLong(req.getParameter("trainNo")));
					ps.setString(2, req.getParameter("fromStn").toUpperCase());
					ps.setString(3, req.getParameter("toStn").toUpperCase());
